...
const supabaseClient = window.supabase.createClient('https://ltebzkjhbcyqkrfefmvd.supabase.co', 'sb_publishable_CbWHJx9Tr4t0Qj6Z5YxvPA_1l0Bgo0m');
const pageSize = 10;
...
function audioUrl(path) { return path ? supabaseClient.storage.from('birthday-voices').getPublicUrl(path).data.publicUrl : null; }
...
async function updateTotal() { const { count } = await supabaseClient.from('birthday_wishes').select('*', { count: 'exact', head: true }); if (count !== null) { $('#wishTotal').textContent = count; $('#wishMessageTotal').textContent = count; } }
...
$('#wishForm').addEventListener('submit', async (event) => { event.preventDefault(); const submit = $('#wishForm button[type="submit"]'), name = $('#nameInput').value.trim(), message = $('#wishInput').value.trim(); if (!name || !message) return; submit.disabled = true; submit.textContent = 'Adding your wish…'; let audioPath = null; try { if (voiceBlob) { audioPath = `${crypto.randomUUID()}.webm`; const { error } = await supabaseClient.storage.from('birthday-voices').upload(audioPath, voiceBlob, { contentType: 'audio/webm', upsert: false }); if (error) throw error; } const { error } = await supabaseClient.from('birthday_wishes').insert({ name, message, audio_path: audioPath }); if (error) throw error; await loadWishes(true); $('#wishForm').reset(); voiceBlob = null; $('#voicePreview').hidden = true; $('#recordButton').textContent = '◉ Add a voice note'; $('#conversation').innerHTML = '<p class="bot-line">Your wish is on the wall. Thank you for being part of my day!</p>'; confetti(); $('#giftPrompt').showModal(); } catch (error) { console.error(error); $('#conversation').innerHTML = '<p class="bot-line">I could not save that yet. Please try again after the guestbook setup is complete.</p>'; } finally { submit.disabled = false; submit.innerHTML = 'Add to card <span>↗</span>'; } });
